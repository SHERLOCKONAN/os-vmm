...
/* FIFO */
#define FIFO "/tmp/vmmFIFO"

/* 页面大小（字节）*/
#define PAGE_SIZE 4
/* 虚存空间大小（字节） */
#define VIRTUAL_MEMORY_SIZE (64 * 4 * 4)
#define VIRTUAL_MEMORY_EACH VIRTUAL_MEMORY_SIZE / MASTER_PAGE_SUM
/* 实存空间大小（字节） */ 
#define ACTUAL_MEMORY_SIZE (32 * 4)
/* 总虚页数 */
#define PAGE_SUM (VIRTUAL_MEMORY_SIZE / PAGE_SIZE)
#define MASTER_PAGE_SUM 4
/* 总物理块数 */
...
{
    PageTableItem *pages;
} MasterPageTableItem, *Ptr_MasterPageTableItem;
...
typedef enum {
    ...
    ERROR_FILE_CREATE_FAILED //文件创建失败
} ERROR_CODE;
...
/* 页面替换 */
void do_switch(Ptr_PageTableItem);
