...
void do_test()
{
	...
	{
		pageList[i].listNum = i;
		for(j = 0; j < LIST_SUM; j++)
		{
			pageList[i].pageTable[j].pageNum = j;
		}
	}
}
...
void do_init()
{
	...
	for (i = 0; i < PAGE_SUM; i++)
	{
		...
		pageTable[i].count = 0;
		pageTable[i].time = 0;
		pageTable[i].processNum = random() % PROCESS_NUM;
		/* 使用随机数设置该页的保护类型 */
		switch (random() % 7)
		{
			case 0:
	...
	for (j = 0; j < BLOCK_SUM; j++)
	{
		...
		if (random() % 2 == 0)
		{
			...
			blockStatus[j] = TRUE;
			//pageTable[j].processNum = random % 10;
		}
		else
			blockStatus[j] = FALSE;
	}
...
{
	unsigned int i, min, page;
	printf("没有空闲物理快，开始进行LRU页面替换...\n");
	for(i = 0,min = cpuTime,page = 0;i < PAGE_SUM; i++)
	{
		if(pageTable[i].time < min)
		{
...
void do_page_in(Ptr_PageTableItem ptr_pageTabIt, unsigned int blockNum)
{
	...
	readNum = fread(actMem + blockNum * PAGE_SIZE, sizeof(BYTE), PAGE_SIZE, ptr_auxMem);
	//printf("this is a test %d\n",readNum);
	//printf("this is a test %d",actMem + ptr_pageTabIt->blockNum * PAGE_SIZE);
	if ((readNum = fread(actMem + blockNum * PAGE_SIZE, 
		sizeof(BYTE), PAGE_SIZE, ptr_auxMem)) < PAGE_SIZE)
	{
#ifdef DEBUG
		//readNum = fread(buf,sizeof(BYTE),PAGE_SIZE,ptr_auxMem);
		//printf("DEBUG:%d\t%s\n",readNum,buf);
		//fseek(ptr_auxMem,0,SEEK_END);
		printf("DEBUG: auxAddr=%lu\tftell=%u\n", ptr_pageTabIt->auxAddr, ftell(ptr_auxMem));
...
void do_error(ERROR_CODE code)
{
	switch (code)
	{
		...
		case ERROR_OVER_PROCESS:
		{
			printf("访存失败：请求与所访问的页表不属于统一进程\n");
			break;
		}
...
void do_request()
{
	...
	int type,write;
	char c;
	printf("是否手动输入请求：Y键手动键入请求，其他键自动产生请求\n");
	if((c = getchar()) == 'y' || c == 'Y')
	{
		printf("请输入请求的地址：\n");
		scanf("%lu",&ptr_memAccReq->virAddr);
		printf("请输入请求的类型，0代表读请求，1代表写请求，2代表执行请求\n");
		scanf("%d",&type);
		switch(type)
		{
			case 0: //读请求
			...
			case 1: //写请求
			{
				...
				printf("您输入的是写请求，请输入要写入的值\n");
				scanf("%d",&write);
				ptr_memAccReq->value = write % 0xFFu;
	...
	else
	{
		...
		/*随机产生请求所属的进程信息*/
		ptr_memAccReq->processNum = random() % PROCESS_NUM;
		/* 随机产生请求类型 */
		switch (random() % 3)
		{
			case 0: //读请求
