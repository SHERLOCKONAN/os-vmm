...
int Aging_counter[BLOCK_SUM][AgingCountBit];
	...
	while (1) {
		if (fpid < 0) {
			...
			if (counter>=LRU_period){//超过刷新时间
				//老化算法计数位全部右移
				for(i=1;i<AgingCountBit;i++){
					for(j=0;j<BLOCK_SUM;j++){
						Aging_counter[i][j] = Aging_counter[(i-1)%8][j];
					}
				...
				{
					if (pageTable[i].filled){
						Aging_counter[0][pageTable[i].blockNum] = pageTable[i].LRU_flag;  
					} 
					pageTable[i].LRU_flag = 0 ;
