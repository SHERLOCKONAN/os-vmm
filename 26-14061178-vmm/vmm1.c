...
void deal_request(char buf[])
{
	...
	switch (buf[0]-'0')
	{
		...
		{
			ptr_memAccReq->reqType = REQUEST_WRITE;
			sscanf(buf,"1 %lu %02X %d",&addr,&value,&pid);
			printf("%lu\n",addr);
			/* 随机产生待写入的值 */
	...
	ptr_memAccReq->FromProgress=pid;
	printf("ptr_memAccReq->FromProgress = %u\n", ptr_memAccReq->FromProgress);
}
