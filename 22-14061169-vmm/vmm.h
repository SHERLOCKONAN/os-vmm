...
#define SECOND_PAGE_SUM 16
//lyc add
#define PROCESS_SUM 2
//lyc ..
//jsz add
#define MAXLONG 9223372036854775807 //定义long类型的最大值
#define DATALEN sizeof(MemoryAccessRequest) //读取数据大小
//...
/* 定义字节类型 */
...
typedef struct
{
	...
	BYTE value; //写请求的值
	//lyc add
	unsigned int pid;//产生访存请求进程的模拟id号
	//lyc ..
	int command;//jsz add x和y指令 0 无指令 -1 y指令 -2 x指令
...
/* LRU页面替换 */
void do_LRU(Ptr_PageTableItem);//jsz add
