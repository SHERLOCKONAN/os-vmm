...
/* 页面大小（字节）*/
#define PAGE_SIZE 4
/* 虚存空间大小（字节） in one second level page*/
...
{
	unsigned int pageNum;
	PageTableItem SecondPageTable[PAGE_SUM];
} FirstLevelPageTable, *Ptr_FirstLevelPageTable;

typedef struct 
{
	unsigned int FirstLevelNumber;
	unsigned int SecondLevelNumber;
} PageRecord;
/* 访存请求类型 */
typedef enum {
	REQUEST_READ,
...
typedef struct
{
	unsigned long virFirstLevelPage;
...
typedef enum {
	...
	ERROR_PROCESS_NUMBER  //不能被指定进程访问
} ERROR_CODE;
