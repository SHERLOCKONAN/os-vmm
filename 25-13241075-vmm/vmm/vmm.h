...
typedef struct
{
	...
	unsigned long time;//页面最近一次使用时间
	unsigned int processNum;//页表所属进程
...
typedef struct
{
	...
	PageTableItem pageTable[LIST_SUM];//含有的二级页表
}PageList;
...
typedef struct
{
	...
	BYTE value; //写请求的值
	unsigned int processNum;//请求所属进程
...
typedef enum {
	...
	ERROR_OVER_PROCESS //页表与请求不在同一进程
} ERROR_CODE;
